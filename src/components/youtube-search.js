...
export class YoutubeSearch extends LitElement {
  ...
  constructor() {
    ...
    this.key = 'AIzaSyAPtr-Yoklg48IUEizoqhtoMQ8dM9H3104';
    this.query = 'TA22';
    this.results = [];
  }

  fetchCommentCounts(ids) {
    return fetch(`https://www.googleapis.com/youtube/v3/videos?key=${this.key}&part=statistics&id=${ids}`)
      .then(r => r.json())
  ...
  async updated(changedProps) {
    ...
    if (
      (changedProps.get('query') !== undefined || changedProps.get('activeSort') !== undefined) &&
      (changedProps.get('query') !== this.query || changedProps.get('activeSort') !== this.activeSort)
    ) {
      this.results = await this.fetchSearchResults(this.query, this.activeSort.toLowerCase());
    }
  }
  ...
  render() {
    ...
    return html `
      <div class="container">
        <header>
          <search-input @updated=${this.handleQueryUpdate} query=${query}></search-input>
          <sort-controls @updated=${this.handleSortUpdate} activeSort=${this.activeSort}></sort-controls>
        </header>
        ${results.length ? html`<video-list .videos=${results}></video-list>` : ''}
    </div>
    `;
  }
}
